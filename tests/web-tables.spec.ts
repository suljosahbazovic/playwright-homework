...
test.describe('Petclinic Full Test Suite', () => {
    ...
    test.describe('Petclinic Web Tables - Owners Search', () => {
        ...
        test('1. Validate the pet name city of the owner', async ({ page }) => {
            ...
            const jeffBlackRowOwner = page.getByRole('row', {name: 'Jeff Black'})
            await expect(jeffBlackRowOwner).toContainText('Monona')
            await expect(jeffBlackRowOwner).toContainText('Lucky')
        })
